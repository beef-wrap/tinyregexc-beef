...
static
{
	typealias TestCase = (bool, char8*, char8*, int);
	...
	public static void matchers()
	{
		...
		for (let t in tests)
		{
			...
			if (succeed)
			{
				Test.Assert(length == correctlen);
			}
		}
	}

	[Test]
	public static void inverted_set()
	{
		/* Test 1: inverted set without a closing ']' */
		Test.Assert(re_compile("\\\x01[^\\\xff][^") == null);
	}

	[Test]
	public static void incomplete_escape()
	{
